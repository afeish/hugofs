...
sudo apt-get update && sudo apt-get install -y ntp numactl yq
...
echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://mirrors.ustc.edu.cn/docker-ce/linux/ubuntu focal stable" > /etc/apt/sources.list.d/docker.list
...
sudo apt update && export DEBIAN_FRONTEND=noninteractive && sudo apt install gaia rclone docker.io docker-compose-plugin make -y

sudo snap install go goreleaser --classic